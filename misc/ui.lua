if not SMODS.GUI.scrollbar then
	SMODS.GUI.scrollbar = function(args)
		...
		if
			args.scroll_collision_obj
			and (not args.scroll_collision_obj.is or not args.scroll_collision_obj:is(SMODS.UIScrollBox))
		then
	...
	G.FUNCS.scrollbar = function(e)
		...
		elseif
			scrollbox and scrollbox:collides_with_point(G.CURSOR.T) or scrollbar_track:collides_with_point(G.CURSOR.T)
		then
			local scroll_velocity = Multiverse.wheel_velocity.y * (e.config.scroll_mult or 1) / G.TILESIZE
	...
	Multiverse.wheel_velocity = { x = 0, y = 0 }
	function Multiverse.update_scroll(dt)
		if math.abs(Multiverse.wheel_velocity.x) > 0.01 then
			Multiverse.wheel_velocity.x = Multiverse.wheel_velocity.x
				- Multiverse.wheel_velocity.x * math.min(dt * 15, 1)
		else
			Multiverse.wheel_velocity.x = 0
		end
		if math.abs(Multiverse.wheel_velocity.y) > 0.01 then
			Multiverse.wheel_velocity.y = Multiverse.wheel_velocity.y
				- Multiverse.wheel_velocity.y * math.min(dt * 15, 1)
		else
			Multiverse.wheel_velocity.y = 0
		end
	end

	local scroll_hook = love.wheelmoved or function(x, y) end
	function love.wheelmoved(x, y)
		scroll_hook(x, y)
		Multiverse.wheel_velocity.x = Multiverse.wheel_velocity.x + x
		Multiverse.wheel_velocity.y = Multiverse.wheel_velocity.y + y
	end
end
