...
asset_dir = r"C:\Users\Alvin\AppData\Roaming\Balatro\Mods\Multiverse\assets"
...
def process_and_clean(input_path, input_directory, upscale_directory):
    relative_path = os.path.relpath(input_path, input_directory)
    output_path = os.path.join(upscale_directory, relative_path)

    os.makedirs(os.path.dirname(output_path), exist_ok=True)

    print("Cleaning image: " + input_path)
    processed_image = process_image(input_path)
    processed_image.save(output_path, "PNG")
...
def process_directory(input_directory, upscale_directory):
    ...
    for root, _, files in os.walk(input_directory):
        for file in files:
            input_path = os.path.join(root, file)
            process_and_clean(input_path, input_directory, input_directory)
