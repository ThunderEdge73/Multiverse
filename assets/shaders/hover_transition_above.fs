...
extern PRECISION vec2 hover_transition_above;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    if (uv.y > progress - 0.03) {
        float percent = (0.03 - abs(progress - uv.y)) / 0.03;
        colour.rgb = colour.rgb * (1 - percent) + percent * purple;
    }
    
    if (hover_transition_above == 2 * hover_transition_above) {
        uv = hover_transition_above;
    }
