...
extern PRECISION vec2 exhausted;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    vec4 SAT = HSL(tex);
    SAT.g = SAT.g*0.5 + 0.000001*exhausted.r;
	SAT.b = SAT.b*0.8;

	tex = RGB(SAT);
	tex.a = tex.a*0.5;

    if ((mod((uv.x + uv.y) * 3.0, 1.0) < 0.25 || mod((uv.x + uv.y) * 3.0, 1.0) > 0.75) && !((uv.x + uv.y) * 3.0 < 0.25 || (uv.x + uv.y) * 3.0 > 5.5)) {
        tex.a = 0.0;
        colour.a = 0.0;
    }

    if (exhausted == 2 * exhausted) {
        uv = exhausted;
    }
